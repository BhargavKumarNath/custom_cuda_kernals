"""Visualization pipeline for Kernel 6 (MoE Top-K Router).

project_plan.md Section 6, with chart 4 adapted to scale across `k`
(top-k experts selected) rather than sequence length/batch — the scaling
dimension unique to this kernel — same adaptation spirit as Kernels 4/5.
"""
...
CSV_PATH = REPO_ROOT / "benchmarks" / "moe_router_results.csv"
OUT_DIR = REPO_ROOT / "visualizations" / "06_moe_topk_router"
...
def plot_speedup_bar(rows: list[dict[str, str]]) -> None:
    shapes = ["mixtral_like", "deepseek_like", "ksweep_k4"]
    shape_labels = {
        "mixtral_like": "T=4096,E=8\nk=2",
        "deepseek_like": "T=4096,E=160\nk=6",
        "ksweep_k4": "T=4096,E=32\nk=4",
    }
    ...
    fig.suptitle("Kernel 6: MoE Top-K Router — Speedup vs. Eager", fontsize=13,
                 color=INK_PRIMARY, y=1.12)
    fig.tight_layout()
    _save(fig, "01_speedup_bar")


def plot_latency_vs_tokens(rows: list[dict[str, str]]) -> None:
    ...
    for ax, dtype in zip(axes, DTYPE_ORDER):
        ...
        for r in rows:
            if r["dtype"] != dtype or not r["case_name"].startswith(sweep_prefix):
                continue
            by_impl[r["impl"]].append((_f(r, "n_tokens"), _f(r, "median_ms")))
        ...
        ax.set_yscale("log")
        ax.set_title(DTYPE_LABEL[dtype], fontsize=11, color=INK_PRIMARY, fontweight="bold")
        ax.set_xlabel("Tokens (log scale), E=8, k=2", fontsize=9, color=INK_SECONDARY)
        _style_axes(ax)
    ...
    fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, 1.04), ncol=3,
               frameon=False, fontsize=9, labelcolor=INK_PRIMARY)
    fig.suptitle("Kernel 6: Execution Latency vs. Token Count (log-log)", fontsize=13,
                 color=INK_PRIMARY, y=1.12)
    fig.tight_layout()
    _save(fig, "02_latency_vs_tokens")
...
def plot_bandwidth_vs_peak(rows: list[dict[str, str]]) -> None:
    ...
    fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, 1.04), ncol=3,
               frameon=False, fontsize=9, labelcolor=INK_PRIMARY)
    fig.suptitle(
        "Kernel 6: Memory Bandwidth (small by construction — router traffic is O(T*E), not a "
        "bandwidth-bound op)", fontsize=11.5, color=INK_PRIMARY, y=1.12,
    )
    fig.tight_layout()
...
def plot_scaling_k(rows: list[dict[str, str]]) -> None:
    ...
    # Zero-floored on purpose: every point here is sub-0.08ms, where CUDA
    # event timing jitter is a large fraction of the signal — an
    # auto-scaled axis would inflate that noise into a misleadingly
    # dramatic-looking curve (see project_plan.md's Kernel 4 chunk_size
    # chart for the same issue/fix).
    ax.set_ylim(0, None)
    ax.set_xlabel("k (top-k experts selected), T=4096, E=32", fontsize=10, color=INK_SECONDARY)
    ...
    ax.set_title("Kernel 6 (CUDA): Scaling vs. k", fontsize=13, color=INK_PRIMARY)
    fig.tight_layout()
...
def main() -> None:
    if not CSV_PATH.exists():
        print(f"error: {CSV_PATH} not found — run benchmarks/moe_router_bench.py first", file=sys.stderr)
        sys.exit(1)
