"""Visualization pipeline for Kernel 3 (Fused RoPE).
...
CSV_PATH = REPO_ROOT / "benchmarks" / "rope_results.csv"
OUT_DIR = REPO_ROOT / "visualizations" / "03_fused_rope"
...
def plot_speedup_bar(rows: list[dict[str, str]]) -> None:
    shapes = ["bs4_seq128_mha", "bs2_seq2048_gqa", "bs1_seq4096_gqa"]
    shape_labels = {
        "bs4_seq128_mha": "B=4,S=128\nMHA,Hd=64",
        "bs2_seq2048_gqa": "B=2,S=2048\nGQA,Hd=128",
        "bs1_seq4096_gqa": "B=1,S=4096\nGQA,Hd=128",
    }
    ...
    fig.suptitle("Kernel 3: Fused RoPE — Speedup vs. Eager", fontsize=13,
...
def plot_latency_vs_size(rows: list[dict[str, str]]) -> None:
    ...
    for ax, dtype in zip(axes, DTYPE_ORDER):
        ...
        ax.set_yscale("log")
        ax.set_title(DTYPE_LABEL[dtype], fontsize=11, color=INK_PRIMARY, fontweight="bold")
        ax.set_xlabel("Tensor size (q+k elements)", fontsize=9, color=INK_SECONDARY)
        _style_axes(ax)
    ...
    fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, 1.04), ncol=3,
               frameon=False, fontsize=9, labelcolor=INK_PRIMARY)
    fig.suptitle("Kernel 3: Execution Latency vs. Tensor Size (log-log)", fontsize=13,
                 color=INK_PRIMARY, y=1.12)
    fig.tight_layout()
...
def plot_bandwidth_vs_peak(rows: list[dict[str, str]]) -> None:
    ...
    fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, 1.04), ncol=3,
               frameon=False, fontsize=9, labelcolor=INK_PRIMARY)
    fig.suptitle("Kernel 3: Bandwidth Utilization vs. Theoretical Peak", fontsize=13,
                 color=INK_PRIMARY, y=1.12)
    fig.tight_layout()
...
def plot_scaling(rows: list[dict[str, str]]) -> None:
    ...
    ax_seq.set_xlabel("Sequence length (rows = S x (Hq+Hkv)), batch=1", fontsize=9, color=INK_SECONDARY)
    ax_seq.set_ylabel("Median latency (ms, log scale)", fontsize=10, color=INK_PRIMARY)
    ax_seq.set_title("Scaling vs. sequence length (GQA 32/8, Hd=128)", fontsize=10.5, color=INK_PRIMARY)
    _style_axes(ax_seq)
    ...
    ax_batch.set_title("Scaling vs. batch size (S=1024, GQA 32/8, Hd=128)", fontsize=10.5, color=INK_PRIMARY)
    _style_axes(ax_batch)

    handles, labels = ax_seq.get_legend_handles_labels()
    fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, 1.04), ncol=3,
               frameon=False, fontsize=9, labelcolor=INK_PRIMARY)
    fig.suptitle("Kernel 3 (CUDA): Scaling Across Sequence Length / Batch Size", fontsize=13,
...
def main() -> None:
    if not CSV_PATH.exists():
        print(f"error: {CSV_PATH} not found — run benchmarks/rope_bench.py first", file=sys.stderr)
        sys.exit(1)
