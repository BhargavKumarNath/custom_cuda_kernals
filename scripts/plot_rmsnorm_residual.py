"""Visualization pipeline for Kernel 1 (Fused RMSNorm + Residual Addition).

project_plan.md Section 6: generates the four standard chart types from
benchmarks/rmsnorm_residual_results.csv into
visualizations/01_fused_rmsnorm_residual/, as both .png and .svg.

Run directly: `python scripts/plot_rmsnorm_residual.py` (after
benchmarks/rmsnorm_residual_bench.py has produced the CSV).
"""
...
CSV_PATH = REPO_ROOT / "benchmarks" / "rmsnorm_residual_results.csv"
OUT_DIR = REPO_ROOT / "visualizations" / "01_fused_rmsnorm_residual"

# --- Validated categorical palette (dataviz skill references/palette.md) ---
# Slots 1-3 (blue/orange/aqua) are the subset validated for all-pairs CVD
# separation, so they're safe to reuse across two different "jobs" below:
# implementation identity (chart 1-3) and dtype identity (chart 4).
...
PEAK_BANDWIDTH_GB_S = 256.0  # RTX 4070 Laptop GPU — see benchmarks/rmsnorm_residual_bench.py

# Chrome (dataviz skill references/palette.md, light mode)
SURFACE = "#fcfcfb"
INK_PRIMARY = "#0b0b0b"
...
# Chart 1: Speedup vs PyTorch Eager & torch.compile (bar chart)
def plot_speedup_bar(rows: list[dict[str, str]]) -> None:
    shapes = ["bs4_seq128_h768", "bs2_seq2048_h4096", "bs1_seq4096_h4096"]
    shape_labels = {
        "bs4_seq128_h768": "B=4,S=128\nH=768",
        "bs2_seq2048_h4096": "B=2,S=2048\nH=4096",
        "bs1_seq4096_h4096": "B=1,S=4096\nH=4096",
    }
    ...
    fig.suptitle("Kernel 1: Fused RMSNorm + Residual — Speedup vs. Eager", fontsize=13,
                 color=INK_PRIMARY, y=1.12)
    fig.tight_layout()
    _save(fig, "01_speedup_bar")


# Chart 2: Execution latency vs tensor size (log-log line)
def plot_latency_vs_size(rows: list[dict[str, str]]) -> None:
    ...
    fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, 1.04), ncol=3,
               frameon=False, fontsize=9, labelcolor=INK_PRIMARY)
    fig.suptitle("Kernel 1: Execution Latency vs. Tensor Size (log-log)", fontsize=13,
                 color=INK_PRIMARY, y=1.12)
    fig.tight_layout()
    _save(fig, "02_latency_vs_size")


# Chart 3: Memory bandwidth utilization vs theoretical peak (scatter)
def plot_bandwidth_vs_peak(rows: list[dict[str, str]]) -> None:
    ...
    fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, 1.04), ncol=3,
               frameon=False, fontsize=9, labelcolor=INK_PRIMARY)
    fig.suptitle("Kernel 1: Bandwidth Utilization vs. Theoretical Peak", fontsize=13,
                 color=INK_PRIMARY, y=1.12)
    fig.tight_layout()
    _save(fig, "03_bandwidth_vs_peak")


# Chart 4: Scaling across sequence length / batch size (line, CUDA kernel)
def plot_scaling(rows: list[dict[str, str]]) -> None:
    ...
    ax_seq.set_title("Scaling vs. sequence length (H=4096)", fontsize=10.5, color=INK_PRIMARY)
    _style_axes(ax_seq)
    ...
    ax_batch.set_title("Scaling vs. batch size (S=1024, H=4096)", fontsize=10.5, color=INK_PRIMARY)
    _style_axes(ax_batch)

    handles, labels = ax_seq.get_legend_handles_labels()
    fig.legend(handles, labels, loc="upper center", bbox_to_anchor=(0.5, 1.04), ncol=3,
               frameon=False, fontsize=9, labelcolor=INK_PRIMARY)
    fig.suptitle("Kernel 1 (CUDA): Scaling Across Sequence Length / Batch Size", fontsize=13,
...
def main() -> None:
    if not CSV_PATH.exists():
        print(f"error: {CSV_PATH} not found — run benchmarks/rmsnorm_residual_bench.py first",
              file=sys.stderr)
        sys.exit(1)
